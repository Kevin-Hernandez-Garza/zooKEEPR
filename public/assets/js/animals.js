...
const getAnimals = (formData = {}) => {
  ...
  .then(response => {
    // checking for any HTTP codes for an Error
    if(!response.ok) {
      return alert('Error: ' + response.statusText);
    }
    // if everything is okay we parse the data into JSON format
    return response.json();
  })

  /* then after that we send our animal array data to the printResults() 
  function where it generates animal cards and prints them out to the page. */
    .then(animalData => {
